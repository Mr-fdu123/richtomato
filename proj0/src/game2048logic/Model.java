    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int x = 0; x < size(); x += 1) {
            for (int y = 0; y < size(); y += 1) {
                Tile t = board.tile(x, y);
                if (x < size() - 1) {
                     Tile t_right = board.tile(x + 1, y);
                     if (t.value() == t_right.value()) {
                        return true;
                 }
                }
                if (y < size() - 1){
                    Tile t_up = board.tile(x, y + 1);
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        while (y < size()){
            if (y == size() - 1){
                targetY = y;
                break;
            } else if (board.tile(x, y + 1) != null && board.tile(x, y + 1).value() == myValue && !board.tile(x, y + 1).wasMerged()) {
                targetY = y + 1;
                score += 2 * myValue;
                break;
            } else if (board.tile(x, y + 1) != null && (board.tile(x, y + 1).value() != myValue || board.tile(x, y + 1).wasMerged())) {
